...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c, return_qkv=False):
...
class DiTEDM(DiT):
    ...
    def forward(self, xt, t, y, augment_labels=None, return_feat=False, out_from_feat=None):
        ...
        t: (B,) tensor of diffusion timesteps
        y: list of text
        return_feat: whether to return features and condition of DiT
        out_from_feat: list, skip deeper features from specific DiT features
        """
        # origin inputs should be float32, cast to specified dtype
        xt = xt.to(self.dtype)
        if self.use_text_encoder:
        ...
        for _, block in enumerate(self.blocks):
            ...
            if not return_feat:
                ...
                x, qkv = auto_grad_checkpoint(block, x, c, return_qkv=True)  # (B, N, D)
                qkvs.append(qkv)
            feats.append(x)
...
class DiTSSL(DiT):
    ...
    def __init__(
        ...
        if dim_ssl > 0:
            ...
            self.feat_regressor = FinalLayer(hidden_size, np.prod(self.patch_size), self.out_channels)
    ...
    def regress_ssl(self, feat, t=None):
        if self.dim_ssl > 0:
            h = self.feat_regressor[0](feat).squeeze(-1)
            return self.feat_regressor[1](h)
        else:
            c_noise = self.c_noise(t)
            t = self.t_embedder(c_noise[..., 0, 0, 0, 0], dtype=feat.dtype)  # (N, D)
            h = self.feat_regressor(feat, t)
            h = self.unpatchify(h)
