...
class DiTCat(nn.Module):
    ...
    def __init__(
        ...
        #     )
        num_conddim = int(condition.split("_")[-1])
        self.x_embedder = PatchEmbed3D(patch_size, in_channels+num_conddim, embed_dim=hidden_size)
    ...
    def forward(self, x, t, y, augment_labels=None):
        ...
        x = x.to(self.dtype)
        if y is not None:
            y = y[:, :, None, None, None].repeat(1, 1, T, H, W)
