...
class CrossAttention(nn.Module):
    def __init__(
        ...
        attn_drop: float = 0.0,
        proj_drop: float = 0.0,
        norm_layer: nn.Module = LlamaRMSNorm,   # Change to your norm layer if needed
        enable_flash_attn: bool = False,
        rope=None,
        qk_norm_legacy: bool = False,
    ) -> None:
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, return_qkv=False) -> torch.Tensor:
        B, N_q, C = query.shape  # Query shape
        _, N_k, _ = key.shape     # Key and Value should have the same shape

        # Flash attention is not memory efficient for small sequences, this is empirical
        enable_flash_attn = self.enable_flash_attn and (N_q > B)
        
        # Compute Q, K, V from query and key/value inputs
        qkv_query = self.qkv(query)
        qkv_key_value = self.qkv(key)
        
        qkv_query_shape = (B, N_q, 3, self.num_heads, self.head_dim)
        qkv_key_value_shape = (B, N_k, 3, self.num_heads, self.head_dim)

        qkv_query = qkv_query.view(qkv_query_shape).permute(2, 0, 3, 1, 4)
        q, k, v = qkv_key_value.view(qkv_key_value_shape).permute(2, 0, 3, 1, 4).unbind(0)

        q = qkv_query[0]  # Only take the query part
        if self.qk_norm_legacy:
            if self.rope:
                q = self.rotary_emb(q)
                k = self.rotary_emb(k)
        ...
        if enable_flash_attn:
            ...
            # (B, #heads, N_q, #dim) -> (B, N_q, #heads, #dim)
            q = q.permute(0, 2, 1, 3)
            k = k.permute(0, 2, 1, 3)
            v = v.permute(0, 2, 1, 3)
            x = flash_attn_func(
...
class TimestepEmbedderEDM(nn.Module):
    """
    Embeds scalar timesteps into vector representations.
    """

    def __init__(self, hidden_size, frequency_embedding_size=16, random_or_learned_sinusoidal_cond=False):
        super().__init__()
        self.frequency_embedding_size = frequency_embedding_size

        if random_or_learned_sinusoidal_cond:
            sinu_pos_emb = RandomOrLearnedSinusoidalPosEmb(frequency_embedding_size, False)
            fourier_dim = frequency_embedding_size + 1
        else:
            sinu_pos_emb = SinusoidalPosEmb(hidden_size)
            fourier_dim = hidden_size

        self.timestep_embedding = sinu_pos_emb
        self.mlp = nn.Sequential(
...
class ParamEmbedderEDM(TimestepEmbedder):
    """
    Embeds scalar timesteps into vector representations.
    """

    def __init__(self, hidden_size, num_conddim, frequency_embedding_size=256, dropout_prob=0):
        super().__init__(hidden_size=hidden_size, frequency_embedding_size=frequency_embedding_size)
        self.mlp = nn.Sequential(
            nn.Linear(frequency_embedding_size*num_conddim if frequency_embedding_size>0 else num_conddim, hidden_size, bias=True),
            nn.SiLU(),
            nn.Linear(hidden_size, hidden_size, bias=True),
        )
        self.frequency_embedding_size = frequency_embedding_size
    ...
    def forward(self, s, train, force_drop_ids=None):
        if s.ndim == 1:
            s = s[:, None]
        assert s.ndim == 2
        use_dropout = self.dropout_prob > 0
        if (train and use_dropout) or (force_drop_ids is not None):
            s = self.token_drop(s, force_drop_ids)
        b, dims = s.shape[0], s.shape[1]
        if self.frequency_embedding_size>0:
            s = rearrange(s, "b d -> (b d)")
            s_freq = self.timestep_embedding(s, self.frequency_embedding_size).to(self.dtype)
            s_freq = rearrange(s_freq, '(b d) f -> b (d f)', b=b, d=dims)
